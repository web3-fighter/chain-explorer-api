...
func (bc *BaseClient) Call(name, module, action, apiUrl string, param map[string]interface{}, outcome interface{}) (err error) {
	if bc.BeforeRequest != nil {
		...
		if err != nil {
			err = xerr.WrapErr(err, "beforeRequest")
			return
		}
	}
	...
	if name == "etherscan" {
		...
		if httpErr != nil {
			err = xerr.WrapErr(httpErr, "http.NewRequest")
			return
		}
	...
	if bc.Verbose {
		...
		if err != nil {
			err = xerr.WrapErr(err, "verbose mode req dump failed")
			return
		}
		fmt.Printf("\n%s\n", reqDump)
		defer func() {
	...
	if err != nil {
		err = xerr.WrapErr(err, "sending request")
		return
	}
	defer func() {
		_ = res.Body.Close()
	...
	if bc.Verbose {
		...
		if err != nil {
			err = xerr.WrapErr(err, "verbose mode res dump failed")
			return
		}
		fmt.Printf("%s\n", resDump)
	}
	...
	if _, err = io.Copy(&content, res.Body); err != nil {
		err = xerr.WrapErr(err, "reading response")
		return
	}
...
func (bc *BaseClient) HandleEtherscanResponse(action string, data []byte, outcome interface{}) error {
	...
	if err != nil {
		err = xerr.WrapErr(err, "json unmarshal etherscan envelope")
		return err
	}
	...
	if err != nil {
		err = xerr.WrapErr(err, "json unmarshal etherscan outcome")
		return err
	}
	return nil
}
...
func (bc *BaseClient) HandleOklinkResponse(data []byte, outcome interface{}) error {
	...
	if err != nil {
		err = xerr.WrapErr(err, "json unmarshal oklink envelope")
		return err
	}
	...
	if err != nil {
		err = xerr.WrapErr(err, "json unmarshal oklink outcome")
		return err
	}
	return nil
}
...
func (bc *BaseClient) HandleSolScanResponse(data []byte, outcome interface{}) error {
	...
	if err != nil {
		err = xerr.WrapErr(err, "json unmarshal solscan envelope")
		return err
	}
	return nil
}
